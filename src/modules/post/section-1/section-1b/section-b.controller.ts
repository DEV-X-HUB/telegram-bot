...
class QuestionPostSectionBController {
  ...
  async sendImageWaitingPrompt(ctx: any) {
    const sender = findSender(ctx);
    if (this.isWaitingImages(sender.id)) await ctx.reply(sectionBFormatter.messages.imageWaitingMsg);
  }
  ...
  async adjustNotifySetting(ctx: any) {
    ...
    await displayDialog(
      ctx,
      sectionBFormatter.messages.notifySettingChanged.concat(` to  ${notify_option.toUpperCase()}`),
    );
    await deleteMessageWithCallback(ctx);
